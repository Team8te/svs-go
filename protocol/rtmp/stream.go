...
// P - Но главная ценность Васи всегда – результат. 
//Он не готов мириться с плохим качеством кода, большим количеством багов.
//Поэтому в его команде тяжело приживаются новички, многие уходят, 
//не выдержав такого давления.
//Но зато те, кто остались, становятся крутыми спецами.

// I - Вася хорошо разбирается в автоматизации процессов разработки,
// но не умеет делать процессы, выходящие за рамки стандартов и best practices.

// e - Про деньги с ним лучше тоже не разговаривать.
// Однажды менеджер продукта пытался дискутировать с ним в этой плоскости,
// но Вася умело свел все в техническую область, и там смог победить.


// Сергей - paEI. Могут не сработаться, есть конфликт внутри
// Лена - paEi
// Егор - pAEi
// Саша - paEi
// Ксения - paeI
...
// Нужно вводить новые процессы
// Обмен опытом - вечера повышения культуры кода
// сколько проектов, что по срокам

/*
Пробелы с софт скилами (отправить на курсы) и фин менеджментом (перенять опыт у продуктов, общение с заказчиками) 
Vladimir Veinskiy говорит:Процессы с точки зрения их построения 
*/
/*
